...
def split_features_and_target(df, target_column='price_mio',random_state=365):
...
def scaling_encoding(X_train_imputed, X_val_imputed, X_test_imputed, num_cols, cat_cols):
...
def FeatureEngineering():
    ...
    X_train, X_val, X_test, y_train, y_val, y_test = split_features_and_target(df)

    # Step 3: Handle missing values
    X_train_imputed, X_val_imputed, X_test_imputed = impute_with_knn(X_train, X_val, X_test, num_cols, cat_cols)

    # Step 4: Preprocess data (scaling & encoding)
    X_train_scaled, X_val_scaled, X_test_scaled = scaling_encoding(X_train_imputed, X_val_imputed, X_test_imputed, num_cols, cat_cols)
